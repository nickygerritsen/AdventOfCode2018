...
if let input: [String] = readInput(day: 4) {
    ...
    let sums = guardData.mapValues { data in
        return data.reduce(0, +)
    }
    if let max = sums.max(by: { $0.value < $1.value }),
       let data = guardData[max.key],
       let maxSleep = data.max(),
       let maxDay = data.firstIndex(where: { $0 == maxSleep }) {
        print(max.key * maxDay)
    }
}